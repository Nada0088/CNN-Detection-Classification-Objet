...
#denir à quel point on veux approfondir la convolution pour extraire les fonctionnalités
...
#et d améliorer les performances du modèle sur de nouvelles données, telles que lensemble de tests de maintien
#la régularisation L2 regles logistiques et le model de réseaux de neurones
#Il existe deux paramètres de régularisation courants :L1 s appelle Lasso et L2 sappelle Ridge
#Ils ont utilisé la perte L1 au lieu de la perte L2 car les valeurs de la tête de régression prévue
...
#Un modèle avec des poids élevés est plus complexe qu un modèle avec des poids plus petits
...
initializer {
#Le but de l'utilisation de la normale tronquée est de surmonter la saturation des fonctions
#temporelles comme sigmoïde (où si la valeur est trop grande / petite, le neurone arrête d'apprendre).
...
#Cette fonction met à jour les poids de sorte que le rapport entre le poids de classication et le poids

#de localisation soit le rapport fourni. De manière arbitraire, le poids de localisation est déni sur 1.0.
...
#Ces composants sont directement transmis aux fonctions de perte dans nos modèles de détection.

#Ces modules sont séparés du modèle principal car les mêmes architectures 
#de prédicteurs de boîtes sont partagées entre plusieurs modèles.

#mask box predictor pour chaque ancre, une prédiction distincte est eectuée pour chaque classe.
#Outre la prédiction de boîtes et de classes, cette classe permet également de prévoir des masques
...
#valeur booléenne si false : le point de contrôle est celui d une classication sinon detection.
...
#le nombre de lots (batchs)(actuellement de taille 1) utilise pour un cycle d'évaluation
